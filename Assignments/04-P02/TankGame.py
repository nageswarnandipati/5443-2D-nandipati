...
Resources = pygame.image.load("D:/Users/revan/Downloads/tank-game-1/tank-game/resources/game_background.png")
...
def tank(x, y, turret_position):
    ...
    pygame.draw.rect(game_layout_display, blue,
                     (x - tnk_height, y, tnk_width, tnk_height))

    pygame.draw.line(game_layout_display, yellow, (x, y),
                     pos_Turrets[turret_position], tur_width)
...
def player2_tank(x, y, turret_position):
    x = int(x)
    y = int(y)
    ...
    pygame.draw.line(game_layout_display, blue, (x, y),
                     pos_Turrets[turret_position], tur_width)
...
def btn(txt, x, y, width, height, inactive_color, active_color, action=None, size=" "):
    ...
    if x + width > cursor[0] > x and y + height > cursor[1] > y:
        pygame.draw.rect(game_layout_display, active_color,
                         (x, y, width, height))
        ...
        pygame.draw.rect(game_layout_display, inactive_color,
                         (x, y, width, height))
...
def barrier(x_loc, ran_height, bar_width):
    pygame.draw.rect(game_layout_display, green, [
                     x_loc, display_height - ran_height, bar_width, ran_height])
...
def playerfireShell(xy, tankx, tanky, turPost, gun_power, xloc, bar_width, ranHeight, eTankX, eTankY):
    ...
    while fire:
        ...
        pygame.draw.circle(game_layout_display, light_red,
                           (startShell[0], startShell[1]), 5)
        ...
        if startShell[1] > display_height - grnd_height:
            ...
            hit_x = int(
                (startShell[0] * display_height - grnd_height) / startShell[1])
...
def player2fireShell(xy, tankx, tanky, turPost, gun_power, xloc, bar_width, ranHeight, ptankx, ptanky):
    ...
    while fire:
        ...
        pygame.draw.circle(game_layout_display, red,
                           (startShell[0], startShell[1]), 5)
...
def game_intro():
    ...
    while intro:
        ...
        msg_screen("By Revanth,Naveen", wheat, 110)
        # msg_screen("Press C to play, P to pause or Q to quit",black,180)

        btn("Play", 150, 500, 100, 50, wheat,
            light_green, action="play", size="vsmall")
        btn("Controls", 350, 500, 100, 50, wheat,
            light_yellow, action="controls", size="vsmall")
        btn("Quit", 550, 500, 100, 50, wheat,
            light_red, action="quit", size="vsmall")
...
def game_over():
    ...
    while game_over:
        ...
        msg_screen("Player 2 Wins!!", wheat, -30)

        btn("Play Again", 150, 500, 150, 50, wheat, light_green, action="play")
        btn("Controls", 350, 500, 100, 50, wheat,
            light_yellow, action="controls")
...
def you_win():
    ...
    while win:
        ...
        game_layout_display.fill(black)
        msg_screen("Game Over", white, -100, size="large")
        msg_screen("Player 1 Wins!!", wheat, -30)
...
def health_bars(p_health, e_health):
    ...
    pygame.draw.rect(game_layout_display, p_health_color,
                     (680, 25, p_health, 25))
    pygame.draw.rect(game_layout_display, e_health_color,
                     (20, 25, e_health, 25))
...
def gameLoop():
    ...
    xloc = (display_width / 2) + random.randint(-0.1 *
    ...
    while not gExit:
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.KEYDOWN:
                if event.key == pygame.K_LEFT:
                    ...
                    damage = player2fireShell(e_gun, eTankX, eTankY, curTurPost2, f_power, xloc, bar_width,
        ...
        if mTankX - (tnk_width / 2) < xloc + bar_width:
            mTankX += 5
        if eTankX -(tnk_width / 2) > xloc - bar_width:
            eTankX -= 5

        game_layout_display.fill(black)
        health_bars(p_health, e_health)
        gun = tank(mTankX, mTankY, curTurPost)
        e_gun = player2_tank(eTankX, eTankY, curTurPost2)
        ...
        game_layout_display.fill(
            green, rect=[0, display_height - grnd_height, display_width, grnd_height])
        pygame.display.update()
